...
const productSendComment = async (productId: string | null, currentUser: string | null, chatMessage: string | null) => {
    ...
    try {
        ...
        console.log(user)
        const ExistChatMessage: ProductCommentType | null = await ProductComment.find({productId: productId}).select("_id listingUserId buyerUserId productId listingChatMessage ExistChatMessage")
